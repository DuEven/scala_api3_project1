...
import util.{RptUtils, Self_MediaUtil}

/**
  * 统计地域指标
  *
  * todo 数据字典  地点有唯一性，，媒体网址，名称等 不具有唯一性
  */
object Re05_Mediatype {
  ...
  def main(args: Array[String]): Unit = {
    ...
    df.rdd.map(row=>{
      ...
      ((  row.getAs[String]("mediatype"),
        (
          //way 01  自定义工具类
          //Self_MediaUtil.ReqPt(row.getAs[Int]("mediatype"))

          //way02   模式匹配
          row.getAs[Int]("mediatype") match {
            case 1 => "长尾媒体"
            case 2 => "视频媒体"
            case 3 => "独立媒体"
            case _ => "其他"
