...
object Log2parquet_self {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 设置压缩方式
    //sparkSession.setConf("spark.sql.parquet.compression.codec","snappy")
    // 处理数据
    //val lines = sparkSession.textFile(inputPath)
    val lines: RDD[String] = sparkSession.sparkContext.textFile(inputPath)

    // todo 设置过滤条件和切分条件 内部如果切割条件相连过多，那么 需要设置切割处理条件
